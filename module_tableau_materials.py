...
#SW test for starship icon thing

   ("starship_icon", 0, "tableau_with_transparency", 1024, 1024, 0, 0, 300, 300,
   [	
       (store_script_param, ":ship_icon", 1),
       (set_fixed_point_multiplier, 100),
      (cur_tableau_set_background_color, 0x000000),
       #TEST(cur_tableau_set_ambient_light, 244,244,122),
 
       (init_position, pos8),
       (position_set_x, pos8, -210),
       (position_set_y, pos8, 200),
       (position_set_z, pos8, 300),
       #TEST(cur_tableau_add_point_light, ":ship_icon", pos8, 244,244,122),
       (cur_tableau_set_camera_parameters, 1, 10, 10, 10, 10000),
       ...
       (init_position, pos1),
    #set different start points so all icons show up without trimming   
	   # (try_begin),
			# (eq, ":ship_icon", "icon_imperial_star_destroyer"),
			# (position_set_x, pos1, -20),
			# (position_set_y, pos1, -20),
			# (position_set_z, pos1, 100),
			
		# (else_try),
			# (eq, ":ship_icon", "icon_imperial_shuttle"),
		...
		# (else_try),
			...
			# (position_set_z, pos1, 100),
		# (try_end),
		#SW - nevermind the above code, all ship models should now be similar sized and just scaled in module_items.py
		(position_set_x, pos1, 10),
		(position_set_y, pos1, -20),
		(position_set_z, pos1, 100),		
       ...
       (cur_tableau_add_map_icon, ":ship_icon", pos1, 0),
      
       (init_position, pos5),
       (position_set_x, pos5, -90),
       (position_set_z, pos5, 400),
       (position_set_y, pos5, 620),
       (position_rotate_x, pos5, -90),
	   (position_rotate_y, pos5, -20),
       (position_rotate_z, pos5, 180),
       (position_rotate_x, pos5, -35),
	   #set zoom distances for different icons since they're not all the same size
	   # (try_begin),
			# (eq, ":ship_icon", "icon_imperial_star_destroyer"),
		...
		# (else_try),
			# (position_move_z, pos5, 200, 0),
		# (try_end),
		#SW - nevermind the above code, all ship models should now be similar sized and just scaled in module_items.py
		(position_move_x, pos5, 20, 0),		# left/right (negative)
		(position_move_y, pos5, 50, 0),		# up (negative)
		(position_move_z, pos5, -200, 0),		# larger positive values (ie. 400) makes the icon smaller
