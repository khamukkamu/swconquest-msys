...
##           (try_end),
      ...
      #swy-- jawas get special jawa robes, unattainable in any other way - Kham: added the other aliens
    ...
    (else_try),
      (eq, ":character_race", tf_geonosian),
      (troop_add_item, "trp_player", "itm_geonosian_armor", 0),
    (else_try),
      (eq, ":character_race", tf_wookiee),
      (troop_add_item, "trp_player", "itm_wookiee_fullbody", 0),
    (else_try),
      (eq, ":character_race", tf_trandoshan),
      (troop_add_item, "trp_player", "itm_trandoshan_armor", 0),
    (else_try),
      (eq, ":character_race", tf_gamorrean),
      (troop_add_item, "trp_player", "itm_gamorrean_armor", 0),
    (try_end),
...
#          (gt, "$encountered_party_hostile", 0),
          ...
          (call_script, "script_party_count_members_with_full_health", "p_main_party"),(ge, reg(0), 3),
           ...
           "Order your troops to attack with your allies while you stay back.",[(neq, "$freelancer_state", 1),(party_set_next_battle_simulation_time, "$g_encountered_party", -1),
...
from     ID_scripts import script_custom_battle_set_faction_troops

 #swy-- sync up this crap with the other lookup table in custom_battle_set_faction_troops by using a state machine.
 #   -- believe me, it was worse before. magic.gif
    ...
    if opcode[0] is assign:
	  ...
	  [
	    ...
	    (try_begin),
	      ...
	      # --
	      (val_sub,      ":cur_troop_amount", 5),

	    #swy-- Click + Shift => Add five
	    (else_try),
	      (key_is_down,  key_left_shift),
	      # --
	      (val_add,      ":cur_troop_amount", 5),

	    #swy-- Click + Ctrl => Remove one if there's at least one
	    (else_try),
	      (ge,           ":cur_troop_amount", 1),
	      (key_is_down,  key_left_control),
	      # --
	      (val_sub,      ":cur_troop_amount", 1),
	      
	    #swy-- Click => Add one
	    (else_try),
	      (val_add,      ":cur_troop_amount", 1),
	    (end_try),
...
game_menus[-1][5] = generated_entries + \
	[
	 ("next", [], "Next",
	  [
	    (try_begin),
...
#+freelancer start
    ...
    [
      ("looter_attack", [
        ...
        [
          ...
          (call_script, "script_setup_random_scene"),
          (modify_visitors_at_site, "$g_scene_to_use"),
          (reset_visitors),
          (set_visitor, 0, "trp_player"),
  ...
  ## Freelancer Looters END
  ## Freelancer Bandits START
  
  ("freelancer_bandits", 0,
    "Your commander has asked {s5} to hunt down bandits near the camp.",
    "none",
    [ (store_troop_faction, ":commander_faction", "$enlisted_lord"),
      (faction_get_slot, ":freelancer_rank", ":commander_faction", slot_freelancer_rank),
      (try_begin),
    ...
    [
      ...
      ("bandit_wounded", [(troop_is_wounded, "trp_player"),], "You are too wounded for this mission.",
        [(change_screen_map)]),
    ]
  ),
  
  ("freelancer_bandit_accept", 0,
    "{s5} found the bandits nearby. You get ready to attack them.",
    "none",
    [ (store_troop_faction, ":commander_faction", "$enlisted_lord"),
      (faction_get_slot, ":freelancer_rank", ":commander_faction", slot_freelancer_rank),
      (try_begin),
      ...
      ("bandit_attack", [
        ...
        [
          ...
          (try_begin),
            (lt, ":level", 6),
            (assign, ":num_looters", 4),
            (assign, ":enemy_troop", "trp_bandit"),
          (else_try),
            (ge, ":level", 15),
            (assign, ":num_looters", ":level"),
            (val_min, ":num_looters", 8),
            (assign, ":enemy_troop", "trp_steppe_bandit"), #Graverobber
          (else_try),
            (assign, ":num_looters", ":level"),
            (val_min, ":num_looters", 8),
            (assign, ":enemy_troop", "trp_bandit"),
      ...
      ("bandit_attack_with_troops", [
        ...
        [
          ...
          (try_begin),
            (lt, ":level", 6),
            (assign, ":num_looters", 8),
            (assign, ":num_friends", 4),
            (assign, ":enemy_troop", "trp_bandit"),
          (else_try),
            (ge, ":level", 15),
            (store_random_in_range, ":rand", 5, 11),
            (assign, ":num_looters", ":level"),
            (val_min, ":num_looters", 26),
            (assign, ":num_friends", ":rand"),
            (val_add, ":num_friends", ":rand"),
            (assign, ":enemy_troop", "trp_steppe_bandit"), #Graverobber
          (else_try),
            (store_random_in_range, ":rand", 5, 11),
            (assign, ":num_looters", ":level"),
  ...
  ("freelancer_bandit_conclusion",0,
    "{s5}",
    "none",
    [
      (try_begin),
